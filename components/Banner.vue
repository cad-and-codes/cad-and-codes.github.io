<template>
  <div class="relative isolate flex items-center gap-x-6 overflow-hidden bg-gray-50 px-6 py-2.5 sm:px-3.5 sm:before:flex-1">
    <!-- Gradient Backgrounds (v‑for loop reduces duplicate code) -->
    <div
      v-for="(gradient, index) in gradients"
      :key="index"
      class="absolute top-1/2 -z-10 -translate-y-1/2 transform-gpu blur-2xl"
      :class="gradient.position"
      aria-hidden="true"
    >
      <!-- Single Gradient Element -->
      <div
    ...
    <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
      <!-- Announcement Text with Separator -->
...
// Gradient positions to make both background elements dynamic
const gradients = [
  { position: 'left-[max(-7rem,calc(50%-52rem))]' },
  { position: 'left-[max(45rem,calc(50%+8rem))]' },
]
</script>